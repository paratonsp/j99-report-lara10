...
class AkapMonthlyReportController extends Controller
{
    ...
    public function getReportData(Request $request, $month, $year, $trip)
    {
        ...
        $sellingData = AkapMonthly::getSelling($param);
    ...
    public function totalKeterisianKursiChart($param, $classInfo, $seatAndClassBookingData)
    {
        ...
        $offDaysByFleet = [];
        foreach (AkapMonthly::getTemporaryOff($param) as $value) {
        ...
        $onDaysByFleet = [];
        foreach (AkapMonthly::getTemporaryOn($param) as $value) {
            $start = new DateTime($value->date);
            $end = new DateTime($value->date_finish);
            $end->modify('+1 day');
            $period = new DatePeriod($start, new DateInterval('P1D'), $end);
    ...
    {
        $classInfo = AkapMonthly::getAkapClassInfoTableWithTemp($param);
    ...
    public function occupancyByBusChart($param, $classInfo)
    {
        ...
        $book_seat = AkapMonthly::getBookByBus($param);
        ...
        $manifestCounts = AkapMonthly::getManifestCountByBus($param);
        $manifestCountByBus = [];
        ...
        if (count($bus_seat) > 0) {
            ...
            $data['doughnut_chart'][0]['manifest_count'] = "";
            $data['doughnut_chart'][0]['chart'] = $this->nullChart('doughnut');
        }
    ...
    public function perbandinganBulanLaluChart($param)
    {
        ...
        $current_month_book = AkapMonthly::getDailyPassengerCounts($current_month_param);
        $current_month_summary = AkapMonthly::getMonthlyIncomeAndSeats($current_month_param);
        ...
        $last_month_book = AkapMonthly::getDailyPassengerCounts($last_month_param);
        $last_month_summary = AkapMonthly::getMonthlyIncomeAndSeats($last_month_param);
    ...
    public function daftarAbsensiBus($param)
    {
        ...
        $daftar_absensi_bus['trip_assign_open'] = AkapMonthly::getTripAssignOpen($param);
        ...
        $daftar_absensi_bus['trip_assign_close'] = AkapMonthly::getTripAssignClose($param);
    ...
    {
        $akap = AkapMonthly::getTicketingSupport($param);
    ...
    {
        $classInfo = AkapMonthly::getAkapClassInfoList($param);
